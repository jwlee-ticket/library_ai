// 뮤지컬 통합 대시보드 JavaScript
// 데이터는 HTML의 <script type="application/json" id="musical-aggregated-data"> 태그에서 로드됩니다.
...
function updateSummaryCards(data) {
    ...
    const tableStartInput = document.getElementById('table-period-start-date');
    const tableEndInput = document.getElementById('table-period-end-date');
    if (data.data_start_date && data.data_end_date && startInput && endInput && tableStartInput && tableEndInput) {
        startInput.value = data.data_start_date;
        endInput.value = data.data_end_date;
        tableStartInput.value = data.data_start_date;
        tableEndInput.value = data.data_end_date;
    } else if (startInput && endInput && tableStartInput && tableEndInput) {
...
function setDefaultDatesForPeriod() {
    ...
    if (periodType === 'daily') {
        ...
        startDate.setDate(startDate.getDate() - daysUntilMonday - 21);
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    const dataScript = document.getElementById('musical-aggregated-data');
    if (dataScript) {
