...
- **Database**: PostgreSQL
- **Frontend**: Django Templates + Tailwind CSS + [daisyUI](https://daisyui.com)
- **Charts**: Chart.js
...
5. Tailwind CSS + daisyUI: `./static/css/install-daisyui.sh` 실행 후 `./static/css/tailwindcss -i static/css/input.css -o static/css/output.css` 로 CSS 빌드 ([상세](docs/daisyui-setup.md))
6. 마이그레이션: `python manage.py migrate`
7. 관리자 계정 생성: `python manage.py createsuperuser`
8. 개발 서버 실행: `python manage.py runserver`
9. CSS 자동 재빌드 (별도 터미널): `./static/css/tailwindcss -i static/css/input.css -o static/css/output.css --watch`
